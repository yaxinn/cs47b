...
public class Heap_origin {
	...
	// number of elements.
	public Heap_origin (int capacity) {
		mySize = 0;
	...
	private void bubbleDown (int k) {
		assertT (k >= 0 && k < mySize, "bubbling " + k + " down");
	...
	private void bubbleUp (int k) {
		assertT (k >= 0 && k < mySize, "bubbling " + k + " up");
	...
	private void exchange (int k1, int k2) {
		assertT (k1 >= 0 && k2 >= 0 && k1 < mySize && k2 < mySize,
	...
	private void check ( ) {
		...
		for (k=0; 2*k+1<mySize-1; k++) {
			assertT (!isGreater(2*k+1,k), "checking left child of " + k);
			assertT (!isGreater(2*k+2,k), "checking right child of " + k);
		}
		if (2*k+1 == mySize-1) {
			assertT (!isGreater(2*k+1,k), "checking left (only) child of " + k);
		}
	}
	
	// Verify an assertTion.
	private void assertT (boolean whatIsassertTed, String condition) {
		if (!whatIsassertTed) {
			System.err.println ("assertTion failed while " + condition);
			throw new RuntimeException ( );
		}
	}
	
	public static void main (String [ ] args) {
		Heap_origin h = new Heap_origin (9);
		for (int i=1; ; i++) {
			try {
				h.add (i);
			} catch (Exception e) {
